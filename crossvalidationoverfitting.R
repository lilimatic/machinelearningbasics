...
#look for overfitting-seems very much like it
x = xy[[1]]$x
X = cbind(1, poly(x, degree = (length(beta) - 1), raw = TRUE))
y = xy[[1]]$y
...
# Plot the errors-test and train
...
#we code the cross-validation algorithm as follows
folds_i = sample(rep(1:n_folds, length.out = n_train))
cv_tmp = matrix(NA, nrow = n_folds, ncol = length(df))
for (k in 1:n_folds) {
  test_i = which(folds_i == k)
  #one set will be left out
  train_xy = xy[-test_i, ]
  test_xy = xy[test_i, ]
  x = train_xy$x
  y = train_xy$y
...
lines(df, te, lwd = 2, col = "steelblue2", lty = 2)
cv_sd = apply(cv_tmp, 2, sd)/sqrt(n_folds)
errbar(df, cv, cv + cv_sd, cv - cv_sd, add = TRUE, col = "lightpink", pch = 19, 
       lwd = 0.5)
lines(df, cv, lwd = 2, col = "lightpink")
points(df, cv, col = "lightpink", pch = 19)
legend(x = "topright", legend = c("Training error", "Test error", "Cross-validation error"), 
       lty = c(1, 2, 1), lwd = rep(2, 3), col = c(gray(0.4), "steelblue2", "lightpink"), 
       text.width = 0.4, cex = 0.85)

#we do it analogously as of CV
...
lines(df, loocv, lwd = 2, col = "red")
legend(x = "topright", legend = c("Training error", "10-fold CV error", "LOOCV error"),
       lty = c(1, 2, 1), lwd = rep(2, 3), col = c(gray(.4), "steelblue2", "red"),
       text.width = .3, cex = .85)